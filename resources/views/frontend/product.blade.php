...
@section('container')    
  ...
  <aside class="relative w-100 h-screen max-h-[30vh] overflow-y-auto bg-gray-50 dark:bg-gray-800" aria-label="Sidebar">
    <h3 class="text-main text-2xl font-medium text-center mt-8 ">category</h3>
    <div class="px-3 py-4 overflow-y-auto rounded bg-gray-50 dark:bg-gray-800">
        <ul class="space-y-2">
            <li>
                <!-- Menu Dropdown 1 -->
                @foreach($categories as $category)
                <button type="button" class="flex items-center w-full p-2 text-base font-semibold text-main transition duration-75 rounded-lg group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700 " aria-controls="{{ $category->id }}" data-collapse-toggle="{{ $category->id }}" >
