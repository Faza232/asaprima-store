      ...
      <div class="flex justify-center space-x-4">
          <div class="relative group">
              <select id="kategori_id" name="kategori_id" class="form-select input block w-48 px-4 py-2 text-gray-800 border rounded-md border-gray-300 focus:outline-none ring-1 ring-black ring-opacity-5">
                  <option value="{{old('kategori_id', $produk->kategori_id)}}" disabled selected hidden>{{old('name', $produk->kategori->name)}}</option>
          ...
          <div class="relative group">
            <select id="subkategori_id" name="subkategori_id" class="form-select input block w-48 px-4 py-2 text-gray-800 border rounded-md border-gray-300 focus:outline-none ring-1 ring-black ring-opacity-5">
                <option value="{{old('subkategori_id', $produk->subkategori_id)}}" disabled selected>{{old('kategori_id', $produk->subkategori->name)}}</option>
            </select>
          </div>
      </div>
        ...
        >Cover Photo
          ...
          <div class="relative mt-2 flex justify-center rounded-lg border border-dashed border-gray-900/25 px-6 py-10">
            ...
            <div class="mb-3 z-20">
                ...
                @if ($produk->image)
                    <img class="img-preview img-fluid mb-3 col-sm-5 d-block" src="{{ asset($produk->image) }}">
                @else
